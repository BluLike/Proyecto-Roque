...
public class CharacterControllerNonUnity : MonoBehaviour, IDataPersistence
{
    ...
    public ParticleSystem coinsParticle; //particulas dinero.
    ...
    IEnumerator VFXCoinsCooldown(bool addCoins)
    {
        if (addCoins == true)
        {
            Material ps = GameObject.Find("CoinEffect").GetComponent<ParticleSystemRenderer>().material;
            ps.color = Color.white;
            ps.SetColor("_EmissionColor", Color.white);
        }
        else if (addCoins == false)
        {
            Material ps = GameObject.Find("CoinEffect").GetComponent<ParticleSystemRenderer>().material;
            ps.color = Color.white;
            ps.SetColor("_EmissionColor", Color.red);
        }
        coinsParticle.Play();
